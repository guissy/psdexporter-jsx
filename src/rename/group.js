...
function isLikeRectName(layer) {
    return layer.name.indexOf('矩形') === 0;
}
/*判断是否矩形 40 x 100 而且填充色为纯色 */
function isLikeRectSizeAndColor(layer) {
    var _a = layer.bounds, x1 = _a[0], y1 = _a[1], x2 = _a[2], y2 = _a[3];
    var w = x2.value - x1.value;
    var h = y2.value - y1.value;
...
/* index 用于命名不重复 */
var hasGrouped = []; //id;
function walkMoveGroup(g, iii) {
    if (iii === void 0) { iii = Math.random(); }
    if (g.layers && g.layers.length > 0 && hasGrouped.indexOf(g.layers[0].id)) {
        ...
        if (g.stems && g.stems.length > 0) {
            for (var j = 0; j < g.stems.length; j++) {
                var stem = walkMoveGroup(g.stems[j], (iii + 1 + j));
                ids.push(stem.id);
            }
        }
        var indexes = [];
        ...
        var stemsGroup = app.activeDocument.activeLayer;
        return stemsGroup;
    }
}
function groupAll(layerSet) {
    // 处理已有图层组 LayerSet
