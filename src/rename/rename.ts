...
var kkk = 999;
function changeLayerName(layer:LayerSet|Document, index) {
    var layers:Array<Layer> = layer.layers;
    if (layers) {
        var item:ArtLayer = null;
        for (var i = 0; i < layers.length; i++) {
            var j = (index - 1) * 1000 + (kkk--);
            item = <ArtLayer> layers[i];
            if (item.visible == false) {
                item.name = 'hide';
                changeLayerName(<LayerSet> layers[i], index - i);
                item.visible = false;
                //item.remove();
