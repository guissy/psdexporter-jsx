        ...
        for (var i = 0; i < length; i++) {
            ...
            if (artLayer.kind == LayerKind.NORMAL) {
                ...
                if (hasMask) {
                    ...
                    let optionObj = new PNGSaveOptions();
                    ...
                    activeDocument.resizeCanvas(new UnitValue(Math.floor(activeDocument.width.value),'px'),
                        new UnitValue(Math.floor(activeDocument.height.value),'px'), AnchorPosition.MIDDLECENTER);
                    activeDocument.resizeCanvas(new UnitValue(activeDocument.width.value,'px'),
                        new UnitValue(activeDocument.height.value,'px'), AnchorPosition.TOPLEFT);
                }
                // Web用保存して閉じる
                let optionObj = new ExportOptionsSaveForWeb();
